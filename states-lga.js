/*
  Arewa Market — Nigeria States & Local Government Areas

  All 36 states + FCT, all 774 official LGAs.
  Used for cascading State -> LGA dropdowns across the app
  (registration, seller application, product forms, and the
  marketplace location filter).

  If you ever spot a wrong LGA name in your own area, this is
  the ONLY file you need to fix — every page reads from here.
*/
...
export const NIGERIA_STATES = Object.keys(NIGERIA_STATES_LGAS).sort();


/*
  Helper: populate a <select> of LGAs based on a chosen state.
  Usage: populateLgaSelect(stateSelectEl, lgaSelectEl)
*/
export function populateLgaSelect(stateSelectEl, lgaSelectEl, selectedLga) {

  const state = stateSelectEl.value;

  lgaSelectEl.innerHTML = '<option value="">Select LGA</option>';

  lgaSelectEl.disabled = !state;
